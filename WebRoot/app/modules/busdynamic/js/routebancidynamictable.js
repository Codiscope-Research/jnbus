		...
		renderer:function(value){
			...
			*/
			var store = new Ext.data.Store({
				// 获取数据的方式
				proxy : new Ext.data.HttpProxy({
					url :'busdynamic.do?reqCode=queryBusBanciExcuteDynamicTable'
				}),
				// 数据读取器
				reader : new Ext.data.JsonReader({},[{
					name : 'rowid'
			...
			function queryBalanceInfo(pForm) {
				...
				timer = setInterval(function() {
					//store.reload();
					var params = pForm.getValues();
					var maxdata=parseInt(0);
					//获取最大的rowid,那是最新的rowid
					store.each(function(record) {
						if(record!=null){
							var eachdata = parseInt(record.get('rowid'));
							if (eachdata > maxdata) {
								maxdata = eachdata;
							}
						}
					});
					params.maxrowid = maxdata;
					store.load({
						add:true,//在结果集后面添加
						params:params//传的参数
					});
